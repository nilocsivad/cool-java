 ...
 * @author Colin
 */
public class IbatisPropertiesWithFolder extends PropertiesWithFolder {
	...
	public void toInterface(List<TableDefine> list, ITableBeanName rule) throws Exception {
		this.toInterface(list, rule, "ibatis/");
	}

	public void toInterface(List<TableDefine> list, ITableBeanName rule, String folder) throws Exception {

		String templatePath = "template/" + "_IAPI.java.vm";

		if (!Velocity.resourceExists(templatePath)) {
			templatePath = "template/" + folder + "_IAPI.java.vm";
		}
		...
		if (Velocity.resourceExists(templatePath)) {
			...
			for (TableDefine td : list) {
				...
				String bean = rule != null ? rule.translateIt(td.name) : td.name;
				File toFile = new File(toFolder, "I" + bean + "Service.java");
	...
	public void toImplement(List<TableDefine> list, ITableBeanName rule) throws Exception {
		this.toImplement(list, rule, "ibatis/");
	}

	public void toImplement(List<TableDefine> list, ITableBeanName rule, String folder) throws Exception {

		String templatePath = "template/" + "_APIDefaultImpl.java.vm";

		if (!Velocity.resourceExists(templatePath)) {
			templatePath = "template/" + folder + "_APIDefaultImpl.java.vm";
		}
		...
		if (Velocity.resourceExists(templatePath)) {
			...
			index = findIt(KEY_PROP, "PATH_PACKAGE_REF");
			String package_ref = PKG_CLASS[index];
			...
			context.put("package_ref", package_ref);
			context.put("package_iapi", package_iapi);
			context.put("package_model", package_model);
			context.put("package_database", package_database);
			...
			for (TableDefine td : list) {
				...
				String bean = rule != null ? rule.translateIt(td.name) : td.name;
				File toFile = new File(toFolder, bean + "ServiceImplement.java");
	...
	public void toXML(List<TableDefine> list, ITableBeanName rule) throws Exception {
		this.toXML(list, rule, "ibatis/");
	}

	public void toXML(List<TableDefine> list, ITableBeanName rule, String folder) throws Exception {


		String templatePath = "template/" + "sql.xml.vm";

		if (!Velocity.resourceExists(templatePath)) {
			templatePath = "template/" + folder + "sql.xml.vm";
		}
